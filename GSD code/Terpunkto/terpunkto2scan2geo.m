function t= terpunkto2scan2geo(inname,indices,scales,subsampleflag,intensityflag,colorflag,numpts_subsample,method,minscale,maxscale)
...
scan_stats=NaN(size(pts_test,1),1*numscales); %HLJ
rough_stat=NaN(size(pts_test,1),1*numscales); %always gonna calc roughness
...
if method==1
    ...
    for i=1:numloop
        ...
        scantemp=NaN(numscales,1); %HLJ
        roughtemp=NaN(numscales,1); %HLJ 2/2023
...
%               scan_index(j,:) = pts_orig(radinds,[18])its 12 above delete
%               this family 

                geotemp(j,:)=calcdesc(eigvals); %convert eigenvalues to geometric descriptors

                %calculate roughness (RMSE from local plane)
                % if length(ind1)>3 ? <3 invalid plane...already done
                % above!
                tcloud = pts_orig(radinds,[xind,yind,zind]);
                %tcloud = [xind(radinds) yind(radinds) zind(radinds)]; %tempcloud object must be made
                tcloud = tcloud-mean(tcloud);%center cloud about 0,0
                [vecs, vals]=eig(cov([tcloud(:,1),tcloud(:,2),tcloud(:,3)]));
                [~, ind] = sort(vals); %get order of eigenvalues, ascending...
                vecs=vecs(:,ind); %...to then sort eigenvectors
                u = vecs(:,1); %minimum eigenvector to be aligned to +Z
                alpha = -1*atan2(u(2), u(1)); % Rotational angle between the projected u on the xy plane and the x-axis
                beta = pi-atan2(sqrt(u(1)^2 + u(2)^2), u(3)); % Rotational angle between the u vector and the z-axis
                Ry = [cos(beta) 0 sin(beta); 0 1 0; -sin(beta) 0 cos(beta)]; %Rotational matrices
                Rz = [cos(alpha) -sin(alpha) 0; sin(alpha) cos(alpha) 0; 0 0 1];
                tcloud=tcloud'; %rotate data
                nc2=Rz*tcloud; %then apply affine transformation (rotation only) 
                nc2=Ry*nc2;
                nc2=nc2'; %rotate cloud back 
                z=nc2(:,3); %z-residuals define roughness
                z2=z.^2;%rm(S)e
                z3=sqrt(z2); %(R)mse
                roughtemp(j,:)=sum(z3); %rms(E)
                ...
                %Calculate scan number feature
                scandist = pts_orig(radinds,scan_ind); %HLJ (collects scan numbers within radius)
                unscans = unique(scandist); %HLJ (unique scans for information entropy)
                scandistlen = length(scandist); %HLJ (can delete, same as 'length(radinds)' which already exists)
                z = zeros(1,length(unscans));%initialize some variables
                p = zeros(1,length(unscans));
                entropy = 0;
                
                for q = 1:length(unscans) %calculate probability of each scan number 
                   z(q) = length(find(scandist==(unscans(q))));
                   p(q) = z(q)/scandistlen;
                end
                
                entropy = -sum(p.*(log(p)/log(length(unscans))));
                entropy(isnan(entropy))=0;
                scantemp(j,:) = entropy; %HLJ
        ...
        scan_stats(i,:) = reshape(scantemp',1,numscales*1); %HLJ 
        rough_stat(i,:) = reshape(roughtemp',1,numscales*1);
...
elseif method==2
    ...
    for i=1:numloop
        ...
        geotemp=NaN(numscales,10);
        slopetemp=NaN(numscales,4);
        roughtemp=NaN(numscales,1);
        scantemp=NaN(numscales,1);
        intenstemp=NaN(numscales,4);
...
t=struct('geom',real(geomstat),'slope',real(slopes),'scan',(scan_stats),'rough',rough_stat); %create structure and add geom/slope features
...
%% functions
    ...
    function y= calcdesc(vecs)
        ...
        planar = (L2-L3)/L1;
        linear = (L1-L2)/L1;
        %curv = L3; %retiring this definition of curvature=L3 HLJ 2/2023
        curv = L1/(L1+L2+L3);
        scatt = L3/L1;
