function gsd = G3Point(name1)
% clearvars;
addpath('Utils','-end');addpath('Utils/quadfit','-end');  addpath('Utils/geom3d/geom3d/','-end'); 

%% Inputs
% param.ptCloudname='Clasts1e7.ply';% 'Mangaweka.ply' 'Otira_1cm_grains.ply' 'Test1_clean_registered.ply'
param.ptCloudname = name1;
% param.ptCloudpathname = path1;
param.iplot=0;
param.isave=0;

% if isempty(param.ptCloudname)==1;[param.ptCloudname,param.ptCloudpathname] = uigetfile('clasts1e7.ply','Select the *.ply point cloud file');end
...
%Parameters tested during trial and error, with parameter ranges indicated by []. 
% param.nnptcloud=[20 400];
% param.radfactor=[0.6 2];
% param.maxangle1=[10 80];
% param.minflatness=[0.1 0.4];
% param.nmin=[50 300];
